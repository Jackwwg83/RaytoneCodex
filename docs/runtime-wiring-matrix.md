...
| Realtime voice and speech | `thread/realtime/start`, `thread/realtime/appendText`, `thread/realtime/appendAudio`, `thread/realtime/appendSpeech`, `thread/realtime/stop` | `--realtime-session-smoke-test`, `--realtime-voices-smoke-test` |
...
| UI surface | Runtime backing | Evidence |
| --- | --- | --- |
| Tool launcher / environment info | `thread/loaded/list`, `thread/metadata/update`, `thread/goal/get`, `environment/info`, `command/exec git`, `gh pr view` | `--environment-smoke-test`, `--loaded-threads-smoke-test`, `--thread-metadata-smoke-test` |
...
| Terminal panel | `command/exec`, `command/exec/write`, `command/exec/resize`, `command/exec/terminate`, `thread/shellCommand`, `thread/backgroundTerminals/list`, `thread/backgroundTerminals/terminate` | `--terminal-stream-smoke-test`, `--terminal-resize-smoke-test`, `--thread-shell-command-smoke-test` |
...
| Account, profile, usage, billing | `account/read`, `account/login/start`, `account/login/cancel`, `account/logout`, `account/usage/read`, `account/rateLimits/read`, `account/rateLimitResetCredit/consume`, `account/workspaceMessages/read`, `account/sendAddCreditsNudgeEmail` | `--account-auth-smoke-test`, `--profile-share-smoke-test`, `--profile-privacy-smoke-test`, `--usage-activity-smoke-test`, `--add-credits-nudge-smoke-test` |
...
| Keyboard shortcuts | `AppCommands`, `SessionStore.commandSurfaceShortcuts`, `turn/start`, `turn/interrupt`, `thread/archive`, `fs/readDirectory`, `command/exec` | `--command-surface-smoke-test` |
...
| Computer control | `configRequirements/read`, `skills/list`, `mcpServerStatus/list`, `windowsSandbox/readiness`, `windowsSandbox/setupStart` | `--windows-sandbox-smoke-test` |
...
| External agent migration | `externalAgentConfig/detect`, `externalAgentConfig/import`, `externalAgentConfig/import/readHistories`, `externalAgentConfig/import/progress`, `externalAgentConfig/import/completed` | `--external-agent-config-smoke-test`, `--external-agent-real-smoke-test` |
